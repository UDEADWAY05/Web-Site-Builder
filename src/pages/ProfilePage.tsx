...
export const ProfilePage = () => {
  ...
  const { formState:{ errors,isValid,isSubmitting },setError } = form 
  ...
  const onSubmit = async ({ name,surname }:{name:string,surname:string,}) => {
    try {
      ...
      if (e instanceof FirebaseError ){
          setError('root', { type:'Firebase_error',message:FirebaseErrorMap.get(e.code) ?? e.message})
      }
  ...
  return (
    ...
    <form onSubmit={form.handleSubmit(onSubmit) } className="px-2 py-4 flex flex-col sm:w-1/2 md:w-1/3 gap-1">
      ...
      <FormInputField form={form} name={'name'} label='Имя' type='text' placeholder='имя' disabled={!isEditing}/> 
      <FormInputField form={form} name={'surname'} label='Фамилия' type='text' placeholder='фамилия' disabled={!isEditing}/> 
      
      {/* Button keys to prevent propagation click on save button after edit button click*/}
      { isEditing 
        ? <Button key='save' variant='default' type='submit' className="w-full" disabled={!isValid || isSubmitting}>Сохранить</Button>
        : <Button key='edit' variant='secondary' onClick={() => setIsEditing(true)} className="w-full hover:bg-gray-300" >Редактировать</Button>
      }

      {isEditing && <Button variant='default' onClick={() => setIsEditing(false)}>Отменить редактирование</Button>}
