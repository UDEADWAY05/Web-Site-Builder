...
export const Login = () => {
    ...
    const onSubmit = async ({ email,password }:{email:string,password:string}) => {  
      try {
        ...
        if (userData){
          dispatch(setUser({ id:user.uid,email,name:userData.name,surname:userData.surname }))
          navigate('/')
        }
      }
      catch (e:unknown) {
    ...
    <Form {...form}>
      ...
      <FormInputField form={form} name={'password'} label='Пароль' type='password' placeholder='password' />
